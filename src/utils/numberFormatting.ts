...
/**
 * Get the locale-specific group separator (e.g., ',' in en-US, '.' in de-DE)
 */
export function getGroupSeparator(): string {
  const parser = createNumberParser()
  return parser.groupSeparator
}

/**
 * Get the locale-specific decimal separator (e.g., '.' in en-US, ',' in de-DE)
 */
export function getDecimalSeparator(): string {
  const parser = createNumberParser()
  return parser.decimalSeparator
}
