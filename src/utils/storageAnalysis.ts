...
/**
 * Check if there's enough space to add a new mission
 */
export async function canAddMission(mission: Mission): Promise<boolean> {
  const stats = await calculateStorageStats()
  const missionSize = estimateMissionSize(mission)
  return stats.remainingBytes >= missionSize
}
...
/**
 * Get detailed storage report for debugging/display
 */
export async function getStorageReport(): Promise<string> {
  ...
  const lines = [
    '=== Storage Report ===',
    `Total missions: ${stats.missionCount}`,
    `Mission data (localStorage): ${formatBytes(stats.totalBytes)}`,
    `Images (IndexedDB): ${formatBytes(stats.imageStorageBytes || 0)} (${stats.imageCount || 0} images)`,
    `Total storage used: ${formatBytes(stats.totalStorageBytes || stats.totalBytes)} (${percentFormatter.format(stats.percentUsed)})`,
    `Average mission size: ${formatBytes(stats.averageMissionSize)}`,
    `Remaining space: ${formatBytes(stats.remainingBytes)}`,
    `Estimated missions remaining: ~${stats.estimatedMissionsRemaining}`,
    stats.quotaAvailable
      ? `Browser quota: ${formatBytes(stats.quotaAvailable)}`
      : 'Browser quota: Unknown (using conservative estimate)',
    `Status: ${status.level.toUpperCase()}`,
    status.shouldWarn ? `⚠️  ${status.message}` : `✓ ${status.message}`,
  ]
