...
describe('CoordinateInputField', () => {
  ...
  describe('Edge cases and validation', () => {
    ...
    it('should handle partial input without emitting', async () => {
      const wrapper = mount(CoordinateInputField, {
        props: {
          modelValue: null,
          type: 'latitude',
      ...
      // Should show formatted partial input with degree symbol
      expect(input.element.value).toBe('N 31°')

      // Should not emit update:modelValue for incomplete coordinate
      const emitted = wrapper.emitted('update:modelValue')
