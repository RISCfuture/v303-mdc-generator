...
import { validateMissionStorage, validateStoredMissions } from '@/utils/validateMissionStorage'
import { serializeMission } from '@/utils/missionStorage'
import type { Mission } from '@/types'

describe('validateMissionStorage', () => {
  const STORAGE_KEY = 'v303-missions-test'
  ...
  describe('validateStoredMissions', () => {
    it('should validate missions from localStorage', () => {
      const mission = createValidMission()
      const serialized = serializeMission(mission)
      const storageData = {
        version: 2,
      ...
      const result = validateStoredMissions(STORAGE_KEY)

      expect(result.valid).toBe(true)
    })

    it('should return error when localStorage is empty', () => {
      const result = validateStoredMissions(STORAGE_KEY)
    ...
    it('should return error when localStorage contains invalid JSON', () => {
    ...
    it('should return error when localStorage contains invalid mission data', () => {
      const invalidData = {
        version: 1, // Wrong version
        missions: [],
      }
