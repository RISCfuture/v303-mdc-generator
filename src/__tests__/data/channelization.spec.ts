...
describe('channelization', () => {
  describe('getDefaultRadioPresets', () => {
    it('should return an array for valid theater, airframe, and radio index', () => {
      ...
      const presets = getDefaultRadioPresets('Afghanistan', 'A-10C_2', 0)
      expect(Array.isArray(presets)).toBe(true)
    })

    it('should return empty array for invalid theater', () => {
      const presets = getDefaultRadioPresets('NonExistentTheater' as Theater, 'A-10C_2', 0)
      expect(presets).toEqual([])
    })

    it('should return empty array for invalid airframe', () => {
      const presets = getDefaultRadioPresets('Afghanistan', 'NonExistentAircraft', 0)
      expect(presets).toEqual([])
    })

    it('should return empty array for invalid radio index', () => {
      const presets = getDefaultRadioPresets('Afghanistan', 'A-10C_2', 999)
      expect(presets).toEqual([])
    })
    ...
    it('should handle negative radio indices', () => {
      const presets = getDefaultRadioPresets('Afghanistan', 'A-10C_2', -1)
      expect(presets).toEqual([])
    })

    it('should return different presets for different radios', () => {
      const presets0 = getDefaultRadioPresets('Afghanistan', 'A-10C_2', 0)
      const presets1 = getDefaultRadioPresets('Afghanistan', 'A-10C_2', 1)
    ...
    it('should return different presets for different theaters with same airframe', () => {
      ...
      const presetsAfghan = getDefaultRadioPresets('Afghanistan', 'A-10C_2', 0)
      const presetsCaucasus = getDefaultRadioPresets('Caucasus' as Theater, 'A-10C_2', 0)
    ...
    it('should handle empty string theater name', () => {
      const presets = getDefaultRadioPresets('' as Theater, 'A-10C_2', 0)
      expect(presets).toEqual([])
    })

    it('should handle empty string airframe name', () => {
      const presets = getDefaultRadioPresets('Afghanistan', '', 0)
      expect(presets).toEqual([])
    })
  })
