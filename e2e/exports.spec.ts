...
async function addValidSteerpoint(page: Page) {
  ...
  // Fill in latitude, longitude, and altitude
  const latInput = page.locator('[aria-label="Latitude"] input').first();
  await latInput.click();
  await latInput.pressSequentially("N 33°30.346'", { delay: 30 });
  await latInput.blur();
  await page.waitForTimeout(200);

  const lonInput = page.locator('[aria-label="Longitude"] input').first();
  await lonInput.click();
  await lonInput.pressSequentially("E 065°50.859'", { delay: 30 });
  await lonInput.blur();
  await page.waitForTimeout(200);
...
test.describe('Mission Export Operations', () => {
  ...
  test('should have export buttons visible in mission editor', async ({ page }) => {
    await page.goto('/');
    ...
    await expect(page.getByRole('button', { name: /Export JSON MDC/ })).toBeVisible();
  });
  ...
  test('should trigger PDF download when clicking Export PDF', async ({ page }) => {
    ...
    expect(download.suggestedFilename()).toContain('briefing.pdf');
  });

  test('should trigger JSON MDC download when clicking Export JSON MDC', async ({ page }) => {
    await page.goto('/');
    ...
    // Wait for the Export JSON MDC button to become enabled
    await expect(page.getByRole('button', { name: /Export JSON MDC/ })).toBeEnabled({ timeout: 10000 });

    // Set up download listener
    const downloadPromise = page.waitForEvent('download');

    // Click export JSON MDC button
    await page.getByRole('button', { name: /Export JSON MDC/ }).click();

    // Wait for download to start
    const download = await downloadPromise;

    // Verify download filename contains DTC.json
    expect(download.suggestedFilename()).toContain('DTC.json');
  });

  test('should export mission with crew members', async ({ page }) => {
    await page.goto('/');
  ...
  test('should export mission with steerpoints', async ({ page }) => {
    await page.goto('/');
  ...
  test('should export mission with loadout', async ({ page }) => {
    await page.goto('/');
    ...
    // Navigate to Loadout tab and select a prefab loadout (test-specific setup)
    await page.getByText('Loadout', { exact: true }).click();
    await page.waitForTimeout(500);

    // Select a prefab loadout - find dropdown within loadout section
    const loadoutDropdown = page.locator('.n-base-selection').first();
    await loadoutDropdown.click();

    // Wait for dropdown menu and select first loadout option
    await page.waitForSelector('.n-base-select-menu', { state: 'visible', timeout: 10000 });
    await page.waitForTimeout(500);
  ...
  test('should show success message after export', async ({ page }) => {
    ...
    // Look for success message (Naive UI uses toast notifications)
    await expect(page.locator('.n-message', { hasText: /generated|success/i })).toBeVisible({ timeout: 5000 });
  });
