...
def create_takeoff_index_regression():
    ...
    print(
        f"    index = {coeffs_multi[0]:.6f} + {coeffs_multi[1]:.6f}*temp_C + {coeffs_multi[2]:.6f}*alt_k + {coeffs_multi[3]:.6f}*temp_C*alt_k"
    )
    ...
    for flap_setting, data, result_key in [
        ...
        for weight, data_points in data.items():
            ...
            if np.sum(valid_mask) >= 3:
                ...
                print(
                    f"    distance = {coeffs[0]:.6f}*idx² + {coeffs[1]:.6f}*idx + {coeffs[2]:.6f}"
                )
        ...
        print(
            f"    distance = {coeffs_multi[0]:.4f} + {coeffs_multi[1]:.4f}*idx + {coeffs_multi[2]:.4f}*idx² + {coeffs_multi[3]:.4f}*wt + {coeffs_multi[4]:.6f}*wt² + {coeffs_multi[5]:.4f}*idx*wt"
        )
...
def visualize_fits():
    ...
    for (alt, indexes), color in zip(alt_data.items(), colors):
        ...
        if np.sum(valid_mask) >= 3:
            ...
            ax1.plot(
                temp_smooth, np.polyval(coeffs, temp_smooth), color=color, linewidth=2, alpha=0.7
            )

    ax1.set_xlabel("Temperature (°C)", fontsize=12)
    ax1.set_ylabel("Takeoff Index", fontsize=12)
    ax1.set_title(
        "Takeoff Index vs Temperature (Selected Altitudes)", fontsize=14, fontweight="bold"
    )
    ...
    for (weight, distances), color in zip(weight_data.items(), colors):
        ...
        if np.sum(valid_mask) >= 4:
            ...
            ax2.plot(
                idx_smooth, np.polyval(coeffs, idx_smooth), color=color, linewidth=2, alpha=0.7
            )

    ax2.set_xlabel("Takeoff Index", fontsize=12)
    ax2.set_ylabel("Takeoff Ground Run (1000 ft)", fontsize=12)
    ax2.set_title(
        "Takeoff Distance vs Index - Flaps 0° (Selected Weights)", fontsize=14, fontweight="bold"
    )
    ...
    ax3.plot(
        wind_smooth, np.polyval(wind_coeffs, wind_smooth), "b-", linewidth=2, label="Quadratic fit"
    )
    ...
    ax4.plot(
        slope_smooth, np.polyval(slope_coeffs, slope_smooth), "g-", linewidth=2, label="Linear fit"
    )
