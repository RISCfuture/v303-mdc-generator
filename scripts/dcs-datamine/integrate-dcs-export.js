...
function processLaunchers(launchersData) {
  ...
  for (const [clsid, data] of Object.entries(launchersData.launchers || {})) {
    ...
    const emptyWeightLbs =
      data.weightEmpty != null ? Math.round(data.weightEmpty * KG_TO_LBS) : null
...
function processAircraft(aircraftData, munitionsDb, diskData) {
  ...
  for (const [id, data] of Object.entries(aircraft)) {
    ...
    console.log(
      `  ✓ ${id} (${data.displayName}) - ${stations.length} stations, ${radios.length} radios`,
    )
  }
...
async function main() {
  ...
  for (const [_name, path] of [
    ['launchers.json', launchersPath],
    ['aircraft.json', aircraftPath],
  ]) {
    if (!existsSync(path)) {
  ...
  if (diskData)
    console.log(
      `  Disk data: ${diskData.radios?.count || 0} radios from ${Object.keys(diskData.radios?.aircraft || {}).length} modules`,
    )
  ...
  writeFileSync(
    join(__dirname, '../../src/data/json/munitions.json'),
    JSON.stringify(munitions, null, 2),
  )
  ...
  console.log(`\n${'='.repeat(60)}`)
  console.log(
    `Done: ${Object.keys(munitions).length} munitions, ${Object.keys(airframes).length} aircraft`,
  )
}
