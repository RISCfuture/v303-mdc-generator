 ...
 * Airband (108-400 MHz range) uses 0.025 MHz steps
...
function calculateStep(min, max) {
  ...
  // Check if this range overlaps with airband (roughly 108-400 MHz range)
  // Airband uses 0.025 MHz steps (25 kHz channels)
  const airbandMin = 108;
  const airbandMax = 400;

  // If the range overlaps with airband, use 0.025 step
  if (max >= airbandMin && min <= airbandMax) {
    return 0.025;
  }
...
async function processAircraft(fileName) {
  ...
  console.log(`    Weight: ${data.emptyWeight} lbs (empty), ${data.maxTakeoffWeight} lbs (max)`);
  console.log(`    Fuel: ${data.internalFuel} lbs`);
  console.log(`    CMDS: ${data.cmdsCapacity} (chaff: ${data.defaultChaff}, flare: ${data.defaultFlare})`);
  console.log(`    Radios: ${data.radios.length}`);
  console.log(`    Guns: ${data.guns.length}`);
